...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut map = diskomap::BTree::open_or_create(file_with_crc32, Some(diskomap::Integrity::Crc32))?;
    ...
    let mut map = diskomap::BTree::open_or_create(blockchain_file, Some(diskomap::Integrity::Sha256Chain(initial_hash)))?;
    map.insert(0, "a".to_string())?;
    map.insert(3, "b".to_string())?;
    map.insert(5, "c".to_string())?;
    drop(map);
